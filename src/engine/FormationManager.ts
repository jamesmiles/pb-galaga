...
/** Set each alive enemy's position from their formation slot. Skip diving, flying-in, and stationary enemies. */
export function updateEnemyPositions(state: GameState): void {
  const formation = state.formation;
  for (const enemy of state.enemies) {
    if (!enemy.isAlive) continue;
    if (enemy.isStationary) continue; // Stationary enemies keep their fixed position
